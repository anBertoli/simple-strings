 ...
 * others by the delimiter. The string iterator should be used with the ss_iter_next
 ...
 * for ss_split_raw(s->buf, del). Look at the documentation of ss_split_raw for
 * further details.
 *
 * Returns ss_iter* or NULL in case of allocation failures.
 */
 ...
 * has already taken care of freeing the string iterator and the ss* must be
...
/*
 * Collect and return all the substrings that a ss_iter could produce. The function is
