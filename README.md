...
terminator at the position `len`. The `cap` field indicates how much space has been allocated
for the string buffer, this space is always >= of the string length. Usually the space allocated
from the library functions is higher than the string length. In this way, when the string grows,
there's no need to perform further reallocations.

The `buf` field is safer to be read and write until the position `len-1`, i.e. one can write to the
string buffer from the position `ss->buf[0]` to `ss->buf[ss->len-1]`. The library functions
automatically handle the allocated space and the fields mentioned above to perform both the needed
operations and maintain the string state consistent. If you manually write into the string buffer
make sure the buffer has enough space (or grow it with the dedicated `ss_grow` function). **The `len`
and `cap` fields must be considered read-only**. 
